...
namespace AprilTags {
    ...
    std::vector<TagDetection> TagDetector::extractTags(const cv::Mat& image) {
        ...
        // convert to internal AprilTags image (: slow, change internally to OpenCV)
...
//            std::cout<<"nEdegs "<<nEdges<<std::endl;
            edges.resize(nEdges);
        ...
        unordered_map<int, vector<XYWeight> > clusters;
        for (int y = 0; y+1 < fimSeg.getHeight(); y++) {
            for (int x = 0; x+1 < fimSeg.getWidth(); x++) {
                ...
                unordered_map<int, vector<XYWeight> >::iterator it = clusters.find(rep);
                if ( it == clusters.end() ) {
        ...
        std::unordered_map<int, std::vector<XYWeight> >::const_iterator clustersItr;
